...

{
  ...
  echo "EGO_RESOURCE_UPDATE_INTERVAL=15"
  echo "EGO_ENABLE_RG_UPDATE_MEMBERSHIP=Y"
  echo "EGO_RG_UPDATE_MEMBERSHIP_INTERVAL=30"
  echo "EGO_LIM_IGNORE_CHECKSUM=Y"
} >> "$EGO_TOP/kernel/conf/ego.conf"

# Update an existing configuration line using sed
sed -i 's/^EGO_DYNAMIC_HOST_WAIT_TIME=.*/EGO_DYNAMIC_HOST_WAIT_TIME=130/' "$EGO_TOP/kernel/conf/ego.conf"
