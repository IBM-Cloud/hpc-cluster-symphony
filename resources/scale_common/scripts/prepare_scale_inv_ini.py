...
def initialize_node_details(
    ...
    if cls_type == "compute":
        ...
        for each_ip in compute_private_ips:
            if compute_private_ips.index(each_ip) <= (
                start_quorum_assign
            ) and compute_private_ips.index(each_ip) <= (manager_count - 1):
            ...
            elif compute_private_ips.index(each_ip) <= (
                start_quorum_assign
            ) and compute_private_ips.index(each_ip) > (manager_count - 1):
                node = {
                    "ip_addr": each_ip,
        ...
        for each_ip in storage_private_ips:
            if storage_private_ips.index(each_ip) <= (
                start_quorum_assign
            ) and storage_private_ips.index(each_ip) <= (manager_count - 1):
            ...
            elif storage_private_ips.index(each_ip) <= (
                start_quorum_assign
            ) and storage_private_ips.index(each_ip) > (manager_count - 1):
                node = {
                    "ip_addr": each_ip,
...
def initialize_scale_config_details(node_classes, param_key, param_value):
    ...
    for each_node in node_classes:
        scale_config["scale_config"].append(
            {"nodeclass": each_node, "params": [{param_key: param_value}]}
        )
...
if __name__ == "__main__":
    ...
    PARSER.add_argument("--default_data_replicas", help="maximum pagepool size in GB")
    PARSER.add_argument("--max_data_replicas", help="maximum pagepool size in GB")
    PARSER.add_argument(
        "--default_metadata_replicas", help="maximum pagepool size in GB"
    )
    PARSER.add_argument("--max_metadata_replicas", help="maximum pagepool size in GB")
    ...
    if (
        ...
        if len(TF["vpc_availability_zones"]) == 1:
            ...
            scale_config = initialize_scale_config_details(
                ["storagenodegrp", "computenodegrp", "computedescnodegrp"],
                "pagepool",
                pagepool_size,
            )
    ...
    if cluster_type in ["storage", "combined"]:
        ...
        scale_storage = initialize_scale_storage_details(
            ...
            disks_list,
            ARGUMENTS.default_metadata_replicas,
            ARGUMENTS.max_metadata_replicas,
            ARGUMENTS.default_data_replicas,
            ARGUMENTS.max_data_replicas,
        )
        with open(
            "%s/%s/%s/%s"
            % (
        ...
        if ARGUMENTS.verbose:
            print(
                "group_vars content:\n%s"
                % yaml.dump(scale_storage, default_flow_style=False)
            )
